...
def query2():
  ...
  for l in links:
    if l.submitter_id == 62443:
      templinks.append(l)
  templinks.sort(cmp=None, key=lambda x: x.submitted_time, reverse=False)
  return templinks

# make the function query() return the number of votes the link with ID = 2 has
def query3():
    c = db.execute("SELECT * FROM links WHERE id=2")
    link = Link(*c.fetchone())
    return link.votes

# make the function query() return the ID of the link that was 
...
# make the function query() return the IDs of the links that were 
# submitted by user 62443 sorted by submission time ascending. 
def query6():
    c = db.execute("SELECT * FROM links " + 
    ...
    return link_ids
def query7():
    c = db.execute("SELECT id FROM links " + 
                   "WHERE submitter_id=62443 " + 
                   "ORDER BY submitted_time ASC")
    link_ids = []
    for id_tuple in c:
      (tuple_id,) = id_tuple    
      link_ids.append(tuple_id)
    return link_ids

# implement the function link_by_id() that takes a link's ID and returns
# the link itself
...
# implement the function build_link_index() that creates a python dictionary
...
def link_by_id_from_index(link_id):
  #return link_index[link_id]
  # using 'get' handles missed lookups without throwing exceptions
  return link_index.get(link_id)

# implement the function add_new_link() that both adds a link to the 
...
def test():
  l = Link(50, 1, 1, 1, "Small", "Small URL")
  add_new_link(l)
  print link_by_id(50)
  print link_by_id_from_index(50)
